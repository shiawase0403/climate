...
import { CloudRain, Map as MapIcon, Loader2, AlertCircle, Waves, Languages, Download, SplitSquareHorizontal, MousePointerClick, X, Trash2, BookOpen, Gamepad2, Trophy, Target, ArrowRight, Lightbulb, Globe, Award, Medal, Flag, HelpCircle, Swords, CheckCircle, Rocket, Import, PlayCircle } from 'lucide-react';
...
const PALETTE = [
  ...
  '#f59e0b', // Amber
  '#8b5cf6', // Violet
  '#ec4899', // Pink (Extra for 6th point)
];
...
const App: React.FC = () => {
  ...
  const handleImportGameRound = async () => {
    ...
    setComparePoints([]); // Clear previous to allow fresh import up to 6
    setError(null);
    ...
    try {
      ...
      // 2. Find Round
      // Matches use 1-based indexing for rounds usually
      ...
      // 3. Prepare Fetch List (Max 6: 1 Target + 5 Players)
      const targets = [];
      
      // Target City (Right Answer)
      targets.push({
      ...
      for (let i = 0; i < total; i++) {
          ...
          try {
             ...
             // Determine color
             // Target gets Green (#10b981), Players cycle through others
             let color;
             if (t.isTarget) {
                color = '#10b981'; // Green
             } else {
                // Use palette but skip green if possible to keep it distinct for target
          ...
          // Delay 800ms to avoid rate limiting
